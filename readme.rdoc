...
= RollCageX version 0.07, <codename here>
=== 
...
First of all, you will need the libraries mentioned above (requirements),
including the development headers. Check if your distro provides development
libraries for ODE and SDL in separate packages and if so, install them too.
Also make sure you got at least a Make program and a C++ compiler (such as GNU
Make and the GNU C++ Compiler, g++) and the usual libraries.

RCX currently does not use autoconf, but only a custom makefile, so in order
...
There is also an experimental windows "DevKit", which should provide everything
needed for compilation. But is not guaranteed to work (it's in testing stage).

Then cd into the src directory and use "make w32" to compile a static binary,
called "rcx.exe", which includes the needed libraries (for easy redistribution).
